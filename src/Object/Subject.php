...
class Subject implements \JsonSerializable, SubjectInterface
{
    ...
    const ATTRIBUTE_PREFERRED_LANGUAGE = 'locale';
...
*/
    ...
    public static function fromModuleResults(?ModuleResultList $moduleResultList)
    {
        ...
        foreach ($moduleResultList->toArray() as $moduleResult) {
            if ($moduleResult->getSubject() != null) {
                ...
                if ($firstSubject == null) {
                    ...
                    $attributesNew = $moduleResult->getSubject()->getAttributes();
        ...
        $result->setTypeIdentifier($firstSubject->getTypeIdentifier());
    ...
    public static function fromJson($json)
    {
        ...
        return (new static($json->identifier))->setUuid($json->uuid)->setIdentifier($json->identifier)->setUserId($json->user_id ?? null)->setAttributes((array) $json->attributes)->setTypeIdentifier($json->type);
    }
    ...
    {
        return $this->getAttribute(self::ATTRIBUTE_PREFERRED_LANGUAGE) ?? (($user = $this->getUser()) != null ? $user->preferredLanguage : null);
    }
    ...
    {
        return $this->getAttribute(self::ATTRIBUTE_ACTIVE) ?? (($user = $this->getUser()) != null ? $user->active : false);
    }
