...
class AuthChain
{
    ...
    public static function buildGraph()
    {
        ...
        foreach (resolve('ArieTimmerman\Laravel\AuthChain\Repository\ModuleRepositoryInterface')->all() as $module) {
        ...
        $chainElements = resolve('ArieTimmerman\Laravel\AuthChain\Repository\ChainRepositoryInterface')->all();
    ...
    public function getNextSteps(ModuleInterface $from = null, ModuleInterface $to, $passive = false)
    {
        ...
        foreach ($predecessors as $pre) {
            ...
            Log::debug(sprintf('%s (%s) has predecessor %s (%s)', $to ? $to->name : 'null', $to ? $to->getIdentifier() : 'null', $pre->name, $pre->getIdentifier()));

            $next = $this->getNextSteps($from, $pre, $passive);

            foreach ($next as $n) {
                Log::debug(sprintf('%s (%s) is a next step to %s (%s)', $n->name, $n->getIdentifier(), $pre->name, $pre->getIdentifier()));
            }
